...
function PostCard({ post, index }: { post: Post; index: number }) {
  return (
    ...
    >
      <Link
        href={`/blog/${post.slug}`}
        className="group block p-6 rounded-xl border border-foreground/[0.04] hover:border-warm/10 bg-surface/30 hover:bg-surface/50 transition-all duration-300"
      >
...
export function BlogList({ posts }: { posts: Post[] }) {
  ...
  return (
    <PageShell title="博客" subtitle="前端、后端、AI 与独立开发随笔" accent="#F59E0B">
      {/* 分类筛选 */}
