...
const categoryKeywords: Record<string, string[]> = {
  profile: ["林志强", "介绍", "技术栈", "能力", "合作", "简历", "履历", "适合"],
...
function scoreChunk(chunk: ChatChunk, question: string, terms: string[]) {
  ...
  const isProfileIntent = /他|林志强|能力|技术栈|合作|适合|简历|履历|介绍/.test(question)
  let score = 0
  ...
  if (chunk.id === "profile.dev_intro" && /介绍|是谁|做什么|林志强|他/.test(question)) score += 14
  if (chunk.id === "profile.identity" && /介绍|是谁|身份|林志强|他/.test(question)) score += 10
